...
bool dfs_bridge(ll pos,ll p,ll q,ll dst)
{
    if(pos==p)
    {
        visited[pos]=1;
...
int main()
{
    ...
    while (Q--)
    {
        ...
        if(A==1)
        {
            ...
            if(bridges.find({D,E})!=bridges.end()||bridges.find({E,D})!=bridges.end())
            {
               
                initialization(N);
                if(dfs_bridge(B,D,E,C))
                {
                    cout<<"da"<<endl;
                }
                else
